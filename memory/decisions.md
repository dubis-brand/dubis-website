...
- Single source of truth `usdToIlsCharge()` + `buildIlsBreakdown()` in `js/main.js`: live rate × **1.03 buffer** (covers PayPal's FX spread on the ILS→USD payout we need for USD-denominated Gelato costs), rounded to **whole shekels**. Every ₪ surface (product cards, cart, checkout, PayPal order, email) routes through it → display === charge to the agora.
