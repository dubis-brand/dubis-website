...
**Impact:** 5 distinct FB visitors did add_to_cart (9 events) but would have shown as `(direct)` if they'd ordered — ROAS/attribution unusable. Note: `campaign_daily_metrics` is an unwired LEGACY table (always empty) — NOT evidence of a dead token. The live spend sync is `?type=sync-meta-spend` → `ad_campaigns` (synced ₪282.41 / 99 clicks / 2,085 imps through 08.08) and marketing cloud-runs log `meta_graph_verified:true` daily. Corrected 2026-08-08 by the /dubis session after cross-checking `agent_runs` + `ad_campaigns`.
...
## 2026-09-12 — DNS WhatsApp bot silent since 2026-09-01 (alive, receives, never replies)
**Symptom:** oren: "סוכן הווטסאפ הפסיק לעבוד". Admin tab showed green (heartbeat alive, bridge_ok, paused=false, cap 50) the whole time. Last `reply` event 2026-08-31 21:26 UTC; every `in` since (02.09 Cellcom ×15, 05.09 + 11.09 direct from oren's LID, 09.09) got no reply and NO error event — the bot only ever logs kinds `in`/`reply`.
**What was verified (Supabase logs + DB, no droplet access):** on 05.09 00:45 and 11.09 20:43 UTC the bot fetched `dubis_products` (200) + `rpc/dubis_bot_admin_summary` (200) within 1s of the incoming message → it passed the stale-guard and reached the generation step, then nothing. Bot restarted 11.09 06:29 UTC and still failed → not stuck memory. `wa_bot_sync` cadence uninterrupted (59-60/h) → single instance, no deploy between 31.08 and 11.09. Catalog (34 products, new #56) loads fine. Gemini agents unaffected (bot uses Claude API). Gmail (dubis.brand): no Anthropic mail; DO invoice normal, droplet up.
**Where the failure must be:** the last two steps on the droplet — Claude API call or bridge send — both invisible from Supabase. Evidence lives ONLY in `/opt/whatsapp/bot/bot.log` (+ `journalctl -u whatsapp-bot/whatsapp-bridge`). Ranked suspects: (1) Claude API — credits exhausted / key revoked / hard-coded model retired; (2) bridge send failing while receive works (whatsmeow 405 client-outdated class); (3) silent exception in prompt build.
**Unexplained side finding:** an external Python process polled `GET wa_bot_state?select=payload` every ~13s with a bad key (401) all August and stopped 2026-09-01 04:32 UTC — not the droplet (sync stream continuous). Likely oren's home PC (pilot remnant/watchdog). 7h after the last good reply. Correlation only.
**Design gaps exposed:** (a) the panel measures liveness, not outcome — 11 days of 100% reply failure looked green; (b) bot.py/main.go are not in any repo (only on the droplet + home PC); (c) no error events → add `kind='error'` + `last_error`/`last_reply_at` in state, and an admin red card for `in`-without-`reply` > 5 min on direct chats.
**ROOT CAUSE (confirmed 2026-09-12 23:29 via DO Web Console, `tail -30 /opt/whatsapp/bot/bot.log`):** every reply since 02.09 11:01 UTC fails with Anthropic `400 invalid_request_error: "Your credit balance is too low to access the Anthropic API. Please go to Plans & Billing to upgrade or purchase credits."` — the bot's Anthropic account ran out of prepaid credits. The bot logs it only to bot.log (`generate_reply error`), never to Supabase. Fix = top up credits at console.anthropic.com → Plans & Billing; no restart needed (next message retries). Prevention: set a low-balance email alert in the Anthropic console + push `kind='error'` events to wa_bot_sync.
**RESOLVED 2026-09-12 21:01 UTC:** oren topped up Anthropic credits; the next incoming message got a reply 3s later (sends_today 2). No restart was needed.
**Access path that worked without SSH:** DigitalOcean → Droplets → dns-whatsapp-agent → Droplet Actions → **Web Console** (browser root shell, works from a phone). Account: dubis.brand@gmail.com, team "My Team".
**Report:** scratchpad `WA_BOT_OUTAGE_2026-09-12.html` (sent to oren). Droplet: `dns-whatsapp-agent` 64.226.91.135, docs in dubis-config `O-output/responses/RESPONSE_dns_agent_infrastructure_2026-08-22_1635.html`.
