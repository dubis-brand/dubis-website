// Ep6 "המסננת" assembly — clips → HE subs (node-canvas RTL, speech-timed) → product beat #42 Cream front+back → music bed (sneaky-snitch, ep4 used lewis-dekalb — music-variety rule 2026-07-27).
// Run from dubis-website root: node scripts/_ep5-build.mjs
// Cue timings live in <scratch>/ep6/cues.json (written after silencedetect + transcript verification).
...
const SP = 'C:/Users/tehar/AppData/Local/Temp/claude/C--Users-tehar-OneDrive-Cladue-Projects-Dubis/a4a196a6-433b-490f-adbc-100739f5b6a3/scratchpad';
const B = `${SP}/ep6/build`; fs.mkdirSync(B, { recursive: true });
const W = 1080, H = 1920;
...
const CUES = JSON.parse(fs.readFileSync(`${SP}/ep6/cues.json`, 'utf8')); // [{clip:'c1', cues:[{t0,t1,text,brand?}]}]
let cueIdx = 0;
for (const clip of CUES) {
  const inputs = ['-y', '-i', `${SP}/ep6/${clip.clip}.mp4`];
...
const linkCue = renderCue('DUBIS · dubis.net/?p=49', true, 'link42');
for (const [face, img] of [['front', `${SP}/ep6/beats/product-49-Cream-front.jpg`], ['back', `${SP}/ep6/beats/product-49-Cream-back.jpg`]]) {
...
ff(['-y', '-i', `${B}/joined.mp4`, '-i', 'videos/il-campaign/_music/fluffing-a-duck.mp3',
  ...
  '-map', '0:v', '-map', '[aout]', '-c:v', 'copy', '-c:a', 'aac', '-b:a', '192k', '-movflags', '+faststart', `${B}/sitcom-ep6-hamasnenet.mp4`], 'music');
console.log('EP5 DONE:', `${B}/sitcom-ep6-hamasnenet.mp4`);
