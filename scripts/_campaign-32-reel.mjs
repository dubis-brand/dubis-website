#!/usr/bin/env node
// Campaign reel for #32 "Activewear for the INACTIVE" — distinct face (face-2), exact product.
// Mirrors _dana-reels-2026-06-28.mjs pipeline. Resumable.
...
const CFG = [
  { key: 'camp-32-tee', face: 'face-2', pid: 32, color: 'White', garment: 'crew-neck t-shirt', age: 50, w: 'man',
    anchor: 'easygoing, bald with a short grey beard, warm half-smile',
    scene: 'on a shaded Tel Aviv balcony on a hot afternoon, holding a glass of iced coffee',
    narration: "They call it activewear. The most active thing I do all day is carry my coffee to the balcony. I'm fifty — I dress for comfort now, and I stopped pretending otherwise." },
];
...
for (const c of CFG){
  ...
  writeFileSync(path.join(OUT,'_campaign-32-results.json'), JSON.stringify(results,null,2));
}
log('=== CAMPAIGN #32 REEL DONE ==='); console.log(JSON.stringify(results,null,2));
