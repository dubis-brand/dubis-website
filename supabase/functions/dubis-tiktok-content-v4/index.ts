...
async function emailOren(product, captions, hashtags, taskId, videoUrl, lateResult) {
  ...
  try { await fetch('https://api.resend.com/emails', { method: 'POST', headers: { 'Authorization': 'Bearer ' + RESEND_KEY, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: 'DUBIS Agents <orders@dubis.net>', to: ['teharlev1976@gmail.com'], subject: '[DUBIS TikTok] ' + slogan + (lateResult?.ok ? ' PUBLISHED' : ''), html }) }); return true; } catch (e) { return false; }
}
