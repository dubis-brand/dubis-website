...
- **Checkout guardrails (post 2026-04-22 catastrophe):** Site label = Gelato SKU verbatim (no COLOR_MAP renames). Capture→external-API flows require auto-refund on BOTH sync error + async webhook. PayPal `onError` is NOT a terminal failure signal. Full detail in `../M-memory/checkout-guardrails.md`.
