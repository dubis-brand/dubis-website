  ...
  async function loadGoalsAndSocial(analyticsData) {
    ...
    // Budget tracking — FB ad spend so far
    const fbAdSpent = 30; // 109.97 ILS ≈ $30
    const totalBudget = 500;
    const spent = fbAdSpent;
    const remaining = totalBudget - spent;
    const roi = monthlyRevenue > 0 ? Math.round((monthlyRevenue / spent) * 100) : 0;
    document.getElementById('budget-spent').textContent = '$' + spent;
    document.getElementById('budget-remaining').textContent = '$' + remaining;
    document.getElementById('budget-roi').textContent = roi > 0 ? roi + '%' : '—';
