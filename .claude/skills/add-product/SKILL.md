...
## 2026-06-09 — autonomous flow hardened (read before touching weekly-slogan-product)
- **The route's `W_TYPE_POOL` excludes `v-neck`/`tank-top`.** The `dubis_products_clothing_type_check` constraint only allows `t-shirt/hoodie/zip-hoodie/long-sleeve/cap/cap-emb`. The route used to weight v-neck/tank-top highest → first cron run 500'd on `insert_failed`. They're also `brand:null` TEMPLATES (Hila-K sizing risk). **To re-add:** (1) give vneck/tanktop a real `brand`+`sku` in `create-gelato-order.js` TEMPLATES, (2) `ALTER ... clothing_type_check` to include the new value, (3) re-add to `W_TYPE_POOL` in `agents/index.ts`. All three or it 500s / ships a random-vendor garment.
- **Self-heal retry.** `gha-pipeline-callback` failed branch auto-retries the pipeline ONCE for `auto_publish` products (re-dispatch `boss-approved-product`, guarded by `product_pipeline_queue.retry_count`) before emailing oren. Gelato preview generation is sometimes transient-slow (2026-06-09: all colors empty on run 1, perfect on a plain re-run) — the retry absorbs that. `create-gelato-drafts.js` preview wait is also 8 attempts ×30s. Only after the retry also fails does oren get ONE failure email; every attempt writes an `agent_runs` row (`side_effects.auto_product_retry/auto_product_failed`).
- **Control surface.** The Boss daily report has a "🤖 קו המוצרים האוטומטי" section reading those `agent_runs` markers — latest auto product + retry count + red callout for genuine failures.
- **⚠️ NEVER hold an auto-product with `auto_publish=false` "to observe".** That silently reverts it to the human visual-approval gate (the "אשר ופרסם" email), which contradicts the no-gate design. The auto-publish callback already emails "🤖 עלה אוטומטית" with the `dubis.net/#product-N` link + a remove button — that IS the notification. (Mistake made + corrected 2026-06-09.)
