...
**Cross-garment parity (verified 2026-06-02):** After reclassifying #3/#25 to `zip-hoodie`, oren asked to confirm the front DUBIS™ logo + back slogan are written EXACTLY as on the t-shirts. They are — *by construction*, because the front logo is ONE shared file (`front_logo_white/dark.png`, consumed by every type via `getDesignFiles()`) and the back is ONE `generateBack()` with vertical centering (no per-garment-type offset since the 2026-05-16 rewrite). Visual check of `product-3-Black-{front,back}.jpg` + `product-25-Navy-{front,back}.jpg` vs `product-5-Black-{front,back}.jpg`: same Impact font, same chest-right logo position, same proportional raised ™, same small-top-line/big-punchword back layout. The zip-hoodie mockups show the **corrected crisp ™**; the t-shirt/pullover mockups are stale (May 16 batch, pre-TM-fix) and still show the old rough ™ — a mockup-freshness gap, NOT a design-logic divergence. Fix = full non-zip catalog mockup refresh (open follow-up), never a per-type hand-edit. See `gelato-operations.md` → "Cross-garment design parity".
